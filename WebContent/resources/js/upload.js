...
function initializeButtons(){
	$(".downloadButton").each(function( index, element) {
		$(this).click(function(){
			var form = $(this).find("form");
			window.location.href = "downloadReport?"+form.serialize();
		});
	});
	...
	$(".deleteButton").each(function( index, element) {
		$(this).click(function(){
			var form = $(this).find("form");
			item = objectifyForm(form.serializeArray());
			$.ajax({
		    	...
		    	if(files.itemList && files.itemList.length-1 < REPORT_ROW_TOTAL){
		            ...
		            .pagination({
		                ...
		                callback: function(data, pagination) {
		                    var dataHtml = template({ itemList: data });
